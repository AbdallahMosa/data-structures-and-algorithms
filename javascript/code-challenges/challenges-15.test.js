...
const screenForNames = (arr) => {
  // Solution code here...
  let reagex=/^(Mr.|Mrs.|Ms.|Dr.)(\s)(\w{1,})/g;

  let array =arr.filter(item=>{
   
  return(item,reagex.test(item) ,item.match(reagex))
...
const detectTicTacToeWin = (board) => {
  ...
  let newArray=[row1,row2,row3,col1,col2,col3,diag1,diag2]


  
 for(let i=0;i<newArray.length;i++){
  if (newArray[i].every(item=>item=="O")){
  return true
  }else if (newArray[i].every(item=>item=="X")){
  return true
  }
  }
