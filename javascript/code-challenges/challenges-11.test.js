...
function transformToLis(obj){
  ...
  x.map((item,i)=> {
    // console. log(`<li>${item[i]}: ${item[i+1]}</li>`)
    newArray.push(`<li>${item[0]}: ${item[1]}</li>`);
  });
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  input.map((element, i) => {
    ...
    element.map(iner => {
      ...
      if (typeof (iner) === 'number' && iner % 5 == 0) {

        let afterPow = Math.pow(2, iner);

        newArray[i].push(afterPow);
      }
    });
  });
  return newArray;
